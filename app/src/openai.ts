...
export async function createStreamingChatCompletion(
	...
	messagesToSend.unshift({
		role: 'system',
		content: (parameters.initialSystemPrompt || defaultSystemPrompt).replace(
			'{{ datetime }}',
...
async function selectMessagesToSendSafely(
	messages: OpenAIMessage[],
	maxTokens: number
) {
	const { ChatHistoryTrimmer } = await import(
		/* webpackPreload: true */ './tokenizer/chat-history-trimmer'
	);
